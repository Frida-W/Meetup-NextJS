...
import { GITHUB_REGISTER_SAVE_FAIL } from "utils/err-code";
...
async function redirect(
  req: NextApiRequest,
  res: NextApiResponse<BaseDataResponse<any>>
) {
  ...
  const accessTokeRes = await requestInstance.post<
    null,
    {
      access_token: string;
    }
  ...
  const gitUserInfo = await requestInstance.get<
    null,
    {
      name: string;
      avatar_url: string;
  ...
  if (!userAuth) {
    ...
    userRecord.nickname =
      gitUserInfo?.login || `用户_${Math.floor(Math.random() * 10000)}`;
